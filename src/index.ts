...
const stylesheet = `
:root{color-scheme:light dark;font:16px/1.5 system-ui,sans-serif}body{margin:0;background:Canvas;color:CanvasText}#lmk-view{box-sizing:border-box;max-width:60rem;margin:auto;padding:2rem}#lmk-status{position:fixed;z-index:10;right:1rem;bottom:1rem;padding:.5rem .75rem;border-radius:.35rem;background:CanvasText;color:Canvas;box-shadow:0 2px 12px #0004}#lmk-status:empty{display:none}h1,h2,h3{line-height:1.2}form,section,article{margin:1.5rem 0}label{display:block;margin:.75rem 0;font-weight:600}input,select,textarea,button{box-sizing:border-box;font:inherit}input,select,textarea{display:block;width:100%;max-width:36rem;margin-top:.3rem;padding:.6rem;border:1px solid GrayText;border-radius:.35rem;background:Canvas;color:CanvasText}textarea{min-height:7rem}button{margin:.5rem .5rem .5rem 0;padding:.6rem 1rem;border:0;border-radius:.35rem;background:Highlight;color:HighlightText;cursor:pointer}button[disabled]{opacity:.55;cursor:wait}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:.55rem;border-bottom:1px solid color-mix(in srgb,CanvasText 20%,transparent)}blockquote,.alert{margin:1rem 0;padding:.75rem 1rem;border-left:.3rem solid Highlight;background:color-mix(in srgb,Highlight 10%,Canvas)}img{max-width:100%;height:auto}code,pre{font-family:ui-monospace,monospace}pre{overflow:auto;padding:1rem;background:color-mix(in srgb,CanvasText 8%,Canvas)}:focus-visible{outline:3px solid Highlight;outline-offset:2px}[aria-busy=true]{opacity:.75}@media(prefers-reduced-motion:reduce){*{animation-duration:.01ms!important;transition-duration:.01ms!important}}
`;
